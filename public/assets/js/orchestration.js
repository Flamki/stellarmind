...
function startLiveRun(task, budget) {
  ...
  document.getElementById('result-status-line').textContent = `Working on: "${task}". Live updates will appear below as each agent responds.`;
...
function trackLiveEvent(ev) {
  ...
  switch (ev.type) {
    case 'orchestrator_plan':
      ...
      document.getElementById('result-status-line').textContent = `Plan ready. Executing ${activeRun.planned || 'multiple'} agent tasks now.`;
      break;
    ...
    case 'agent_call': {
      ...
      if (!activeRun.startedAgents.has(agentKey)) {
        ...
        document.getElementById('result-status-line').textContent = `${ev.agent || 'Agent'} is running now...`;
      }
      break;
    }

    case 'x402_payment':
      appendResultLog(`x402 payment settled for ${ev.agent || 'agent'} (${ev.amount || '?'} ${ev.currency || ''})`);
      break;
    ...
    case 'budget_limit':
      ...
      document.getElementById('result-status-line').textContent = 'Budget limit reached for one of the planned agents.';
      break;
