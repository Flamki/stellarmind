 ...
 *
 * Set PROVIDER=anthropic|openai in .env (default: anthropic).
 */
...
const PROVIDER_NAMES = Object.keys(REGISTRY)
...
export function getProvider() {
  ...
  if (!REGISTRY[name]) {
    console.warn(`[provider] Unknown "${name}", falling back to anthropic. Available: ${PROVIDER_NAMES.join(', ')}`)
  ...
  if (!v.valid) {
    console.error(`[provider] "${name}" missing: ${v.missing.join(', ')}. Using anthropic.`)
    _cached = REGISTRY.anthropic()
  }
  return _cached
}
...
export function listProviders() {
  return PROVIDER_NAMES.map((n) => {
    const p = REGISTRY[n]()
    return { name: n, available: p.isAvailable ? p.isAvailable() : true, capabilities: p.capabilities }
  })
}
