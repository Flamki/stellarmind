...
function assertValidationError(err, field, messageFragment) {
  ...
  assert(err.details.some(detail => detail.field === field), `Expected details to include field ${field}`);
...
err = runMiddleware(validateOrchestrate, { method: 'POST', body: { task: 'Build a demo', budget: -1 } });
assertValidationError(err, 'budget', 'Invalid orchestrate request');

err = runMiddleware(validateOrchestrate, { method: 'POST', body: { task: 'Build a demo', budget: 'not-a-number' } });
assertValidationError(err, 'budget', 'Invalid orchestrate request');
...
err = runMiddleware(validatePremiumQuery, { path: '/api/premium/summarize', query: { text: 'x'.repeat(6000) } });
