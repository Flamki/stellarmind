...
Object.entries(pricingConfig.endpoints).forEach(([endpoint, info]) => {
  ...
  assert(priceRegex.test(info.price), `Endpoint ${endpoint} has invalid price format: ${info.price}`);
});
...
Object.entries(pricingConfig.endpoints).forEach(([endpoint, info]) => {
  ...
  assertEqual(pricingConfig.byAgent[info.agent].endpoint, endpoint, `byAgent endpoint mismatch for ${info.agent}`);
});
...
Object.entries(pricingConfig.endpoints).forEach(([endpoint, info]) => {
  ...
  assert(
    isRelated,
    `Endpoint ${endpoint} should relate to agent ${info.agent}`
  );
});
...
console.log(`  Unique Agents: ${new Set(Object.values(pricingConfig.endpoints).map(i => i.agent)).size}`);
console.log(`  Price Points: ${Object.keys(priceDistribution).length}`);
console.log(`  Total Revenue per Call: $${Object.values(pricingConfig.endpoints).reduce((sum, info) => sum + parseFloat(info.price.slice(1)), 0).toFixed(2)}`);
